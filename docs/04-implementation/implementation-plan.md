...
## Trạng thái công việc còn mở (cập nhật 2026-07-20)

> Tóm tắt nhanh để đọc lướt; chi tiết vẫn nằm ở từng task bên dưới. Đây **không** phải cấu trúc lại — chỉ là bảng chỉ đường.
...
- **🟡 Đang code / còn mở (không đóng ở phiên này):** →6 Enforce CSP · →9 Auth HttpOnly cookie · →10 Staging + WAF.
- **🟠 Manual ops / hạ tầng trả phí (defer đến ngân sách/domain):** nâng plan Render + backup/PITR + test khôi phục + diễn tập rollback (→2) · 3 DSN Sentry + 2 monitor UptimeRobot (→5) · cron ngoài gọi `publish-scheduled` (ED-08) · HTTPS + domain + DNS · Lighthouse/k6 trên staging · checklist nghiệm thu go-live. **Go-live vẫn BLOCKED/DEFERRED** (G7-M1).
- **📌 Backlog nội dung (chờ công ty cấp text/ảnh):** câu 4 (vị trí tuyển dụng) · câu 5 (chính sách NS/đào tạo) · xác nhận hết placeholder · ảnh thật cho các dự án.
...
6. ~~**Gửi email form liên hệ**~~ — **✅ ĐÃ XONG trên production**: dùng **Resend API** (`MAIL_PROVIDER=resend`). Đã test thật trên production, email thông báo lead gửi thành công và hộp thư công ty (Gmail) nhận được. **Cập nhật 2026-07-20:** sau khi Resend chạy ổn định, **SMTP fallback đã được gỡ khỏi code backend**, biến `SMTP_*` đã gỡ khỏi Render và Gmail App Password đã thu hồi — email liên hệ production nay **chỉ dùng Resend** (không còn phương án dự phòng SMTP). Chi tiết: [SMTP-REMOVAL-ENV-CLEANUP](../08-audits-and-reports/current/2026-07-20-smtp-removal-env-cleanup.md).
