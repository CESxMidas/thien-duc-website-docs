...
- [ ] **Sentry DSN + UptimeRobot monitors** — tạo 3 project Sentry + dán 3 DSN (`SENTRY_DSN` / `VITE_SENTRY_DSN` / `VITE_SENTRY_DSN`) vào Render/Vercel rồi redeploy; tạo 2 monitor UptimeRobot (`/api` + trang chủ). Checklist: [monitoring-and-alerting](../07-deployment/monitoring-and-alerting.md). (→5)
