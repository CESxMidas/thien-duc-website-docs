...
- [x] **(→ 13) G4 — Baseline đo lường production (Rich Results + PageSpeed)** — *Hạng mục: Performance & SEO* ✅ 2026-07-17 (phần ĐO hoàn tất — có caveat; các fix phát sinh là task riêng, chưa làm)
  ...
  - **3 task ứng viên phát sinh (chưa làm, xem bảng "Đề xuất fix code" trong audit note):** (1) điều tra NO_FCP/NO_LCP dưới Lighthouse — nghi animation/initial-paint, ưu tiên trung bình-cao vì công cụ đo của Google không chấm ổn định được site; (2) trang chủ mobile LCP 4.0s + Speed Index 10.0s; (3) TBT 368ms trang dự án VI mobile (chỉ theo dõi).
