...
6. ~~**Gửi email form liên hệ**~~ — **✅ ĐÃ XONG trên production**: dùng **Resend API** (`MAIL_PROVIDER=resend`). Đã test thật trên production, email thông báo lead gửi thành công và hộp thư công ty (Gmail) nhận được. SMTP vẫn còn nguyên làm phương án dự phòng (`MAIL_PROVIDER=smtp`).
...
- [x] **[MANUAL-OPS]** Cấu hình gửi email thông báo lead trên production — **✅ ĐÃ XONG bằng Resend API** (`MAIL_PROVIDER=resend`). Gmail SMTP đã bỏ vì kết nối SMTP outbound trên Render **timeout** (Render chặn/không ổn định cổng SMTP). Backend nay hỗ trợ `MAIL_PROVIDER=smtp | resend`; production đặt `resend`, còn SMTP giữ nguyên làm phương án dự phòng. Chỉ là nhập cấu hình (không phải việc code); giá trị thật (`RESEND_API_KEY`…) nhập trực tiếp ở Render, không lưu trong Git.
  > ✅ **Ghi nhận hoàn thành (2026-07-20)** — provider: **Resend** · kiểm thử production: **PASS** · email nhận được: **có** (hộp thư Gmail công ty) · phương án dự phòng: **SMTP giữ lại** (`MAIL_PROVIDER=smtp`). Chi tiết: [EMAIL-RESEND-CLOSEOUT](../08-audits-and-reports/current/2026-07-20-email-resend-closeout.md).
