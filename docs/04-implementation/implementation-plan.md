...
## Section 0 — Tóm tắt hiện trạng / cổng production-readiness

> Tóm tắt nhanh để đọc lướt; chi tiết đầy đủ nằm ở từng task trong **Section 6 — Kho lưu trữ hoàn tất / changelog** bên dưới.
> **Cập nhật cấu trúc: 2026-07-20 (PLAN-RESTRUCTURE-M1)** — sắp xếp lại theo thứ tự làm việc thật xuyên suốt 8 nhóm audit; **không xoá task nào**, các mục còn mở được chuyển lên đầu (Section 1–5), lịch sử hoàn tất giữ nguyên ở Section 6.

- **8 nhóm audit:** 1 Architecture & engineering depth · 2 Content operations · 3 Design & UX quality · 4 Performance & SEO · 5 Reliability & security · 6 Integrations & lead capture · 7 Deployment & operations · 8 Process & deliverables.
- **🟠 Cổng go-live: BLOCKED / DEFERRED (2026-07-19, G7-M1).** Hạ tầng còn Free / chưa xác nhận (backend + Postgres Free, backup/PITR + restore + monitoring chưa làm). Hoãn có chủ đích; nâng plan trả phí + monitoring **bắt buộc trước go-live cuối**. Checklist đầy đủ: **Section 3**. Chi tiết: [G7-M1 note](../08-audits-and-reports/current/2026-07-19-g7-m1-manual-ops-verification.md).
- **✅ Vừa đóng (2026-07-20):**
  - **Email liên hệ = Resend-only** — SMTP fallback đã gỡ khỏi code, `SMTP_*` gỡ khỏi Render, Gmail App Password thu hồi. Chi tiết: [SMTP-REMOVAL-ENV-CLEANUP](../08-audits-and-reports/current/2026-07-20-smtp-removal-env-cleanup.md).
  - **ENV cleanup (Nhóm 8 — Process/deliverables):** `backend/.gitignore` siết để bỏ qua `.env*` (giữ `.env.example`); đã gỡ file `admin/.env` trùng ở local; không còn `.env`/`.env.local` nào bị track. Chi tiết: cùng note trên.
  - **Nội dung song ngữ JSONB** (`[~]`→`[x]`) — đã đóng qua →4 cho phạm vi production/audited.
  - **FRONTEND-UI-POLISH-M1 (Nhóm 3 Design & UX + Nhóm 4 Performance & SEO)** — đã **commit** vào `thien-duc-website-frontend` (`85d5fb1 "Fix UI UX FRONT END"`, xác nhận `git status` sạch 2026-07-20). Chi tiết các đợt polish giữ trong Section 6.
- **Điều hướng:** Section 1 (đang làm) · Section 2 (code kế tiếp) · Section 3 (manual ops / production readiness) · Section 4 (security/infra defer) · Section 5 (backlog nội dung) · Section 6 (kho hoàn tất) · Section 7 (backlog tùy chọn).

## Section 1 — Đang làm / in-progress

> Không có task **code** nào đang mở dở ở thời điểm tái cấu trúc. Repo sạch (đã kiểm `git status` cả 5 repo 2026-07-20; `thien-duc-website-resources` chỉ có 1 ảnh untracked không liên quan). FRONTEND-UI-POLISH-M1 đã commit (`85d5fb1`).

- Chưa có hạng mục code đang chỉnh dở. Ứng viên bắt đầu ngay: xem **Section 2** — **→6 SEC-CSP-ENFORCE-M1** là task code khuyến nghị kế tiếp.

## Section 2 — Code kế tiếp (next coding tasks)

- [ ] **(→ 6) Enforce CSP (bỏ Report-Only)** — *Nhóm 5 Reliability & security* · **phiên khuyến nghị: SEC-CSP-ENFORCE-M1**
  ...
  - **Gợi ý:** Theo dõi report một thời gian → chuyển sang `Content-Security-Policy`; thay `unsafe-inline`/`unsafe-eval` bằng nonce/hash của Next.js. Làm **sau** →5 (monitoring) để bắt vi phạm. Khi enforce phải thêm domain ingest Sentry vào `connect-src` (ghi trong `monitoring-and-alerting.md`). Test `next.config.spec.ts` phải cập nhật theo header mới.

### Phiên khuyến nghị tiếp theo (Next recommended sessions)

1. **SEC-CSP-ENFORCE-M1** — enforce CSP (→6 ở trên).
2. **ADMIN-ITEM-FIELDS-M1** — *có vẻ đã đóng* (mục cần xác nhận): 3 ô còn thiếu của form hạng mục (`description`/`highlights`/`quickFacts`) đã được xử lý qua **ADMIN-ITEM-CONTENT-P2 Batch M1** (kiểm chứng runtime A–F, 2026-07-19 — xem Section 6). **Cần xác nhận** không còn ô nào thiếu; nếu còn → mở lại task này. (Mục mơ hồ, ghi trong báo cáo validation.)
3. **OPS-SENTRY-UPTIME / PROD-READINESS-M1** — thao tác manual ops (dán DSN Sentry, tạo monitor UptimeRobot, nâng plan Render, restore/rollback drill…). Chi tiết đầy đủ: **Section 3**.

## Section 3 — Manual ops / production readiness (một checklist)

> Gom mọi thao tác tay ngoài repo cần làm trước go-live cuối — repo không làm thay được (Dashboard Render/Vercel/Sentry/UptimeRobot, DNS). **Go-live BLOCKED/DEFERRED cho tới khi xong** (G7-M1). Chi tiết task nguồn (→2, →5, ED-08, UAT) giữ trong Section 6.

- [ ] **Render paid plan + backup/PITR + restore drill + rollback drill** (→2, phần Dashboard): áp plan trả phí (`render.yaml`: web `starter` always-on, Postgres `basic-256mb`) + thanh toán khi sync Blueprint; nếu free→paid không nâng tại chỗ → di trú theo runbook **trước mốc 90 ngày**; xác nhận backup daily/retention/PITR + điền nhật ký; kiểm thử khôi phục 1 lần; diễn tập rollback 1 lần. Runbook: [backup-and-restore.md](../07-deployment/backup-and-restore.md) · [rollback-plan.md](../07-deployment/rollback-plan.md) · deployment-guide mục 1b.
- [ ] **Sentry DSN + UptimeRobot monitors** (→5, phần Dashboard): tạo 3 project Sentry + dán 3 DSN (`SENTRY_DSN` / `NEXT_PUBLIC_SENTRY_DSN` / `VITE_SENTRY_DSN`) vào Render/Vercel rồi redeploy; tạo 2 monitor UptimeRobot (`/api` + trang chủ). Checklist: [monitoring-and-alerting.md](../07-deployment/monitoring-and-alerting.md).
- [ ] **ED-08 — cron ngoài gọi `publish-scheduled`:** Render free ngủ sau 15 phút nên `@Cron` nội bộ không đủ; đặt UptimeRobot/cron-job.org gọi `POST /api/news/publish-scheduled` (cần token ADMIN). Nếu đã nâng plan always-on ở →2 thì cron nội bộ chạy được — vẫn nên có cron ngoài để chắc.
- [ ] **DNS / HTTPS / domain chính thức** — **(chờ input câu 10)**.
- [ ] **Áp migration production có chủ ý:** `.env` dev đang trỏ Render → trước go-live chạy `prisma migrate deploy` lên Render (full-text search `20260710120000_add_fulltext_search` + `20260711120000_add_cooperation_projects` chưa áp production). Chạy lại các seed đã sửa nếu cần (`SEED_CONFIRM_PRODUCTION=yes`).
- [ ] **UAT thủ công** với Editor + Admin thật trên toàn luồng (đăng nhập, soạn bài, duyệt, form liên hệ, upload ảnh) — gồm bấm tay `BannerFormDialog` / `PageFormDialog` / `BilingualField` (mới chỉ qua `tsc` + `build`, chưa chạy thật với backend + DB + đăng nhập).
- [ ] **Chạy checklist nghiệm thu go-live đầy đủ** (mục 3.3 báo cáo / mục 4 `ke-hoach-thien-duc.md`).
- [ ] **k6 / load test** 50–100 người đồng thời (CL-04, CL-05) + Lighthouse trên staging (Performance CL-02, SEO ≥ 90 CL-10) + test khôi phục backup DB (CL-09).
- [ ] **Xác nhận nội dung không còn placeholder** (dự án, tin tức, tuyển dụng, pháp lý) — **(chờ input câu 1–8)**; liên đới **Section 5**.

## Section 4 — Deferred security / infra tasks

> Giữ mở; phụ thuộc ngân sách / đánh đổi độ phức tạp. Không chặn go-live cơ bản.

- [ ] **(→ 9) Auth bằng cookie HttpOnly** — *Nhóm 5 Reliability & security*
...
- [ ] **(→ 10) Môi trường staging + WAF** — *Nhóm 7 Deployment & operations*
...
## Section 5 — Backlog nội dung / nội dung tương lai

> Chờ công ty cấp text/ảnh. Không chặn tiến độ code. Năng lực code/CMS đã sẵn sàng — chỉ còn nhập liệu.

- [~] **Câu 4 — tuyển dụng / `openPositions`** — *Nhóm 2 Content operations*: `data/careers.ts` + trang `tuyen-dung` đã dựng khung đầy đủ (giá trị, danh sách vị trí, quy trình 3 bước, nộp hồ sơ qua email). `openPositions` **cố ý để rỗng** — chưa có vị trí thật; trang hiện trạng thái "chưa có vị trí đang tuyển" và vẫn `noindex`. Đổ dữ liệu vào mảng đó là trang tự hiện.
- [ ] **[FUTURE-CONTENT — không phải việc code] Câu 5 — chính sách nhân sự / đào tạo / sơ đồ tổ chức** — *Nhóm 2*: 3 trang `dao-tao`, `chinh-sach-nhan-su`, `so-do-to-chuc-cong-ty` đã dựng khung + i18n (EN-SITE-WIDE F3), giữ `noindex`; đổ nội dung thật là trang tự hiện. Backlog nội dung tương lai, không chặn tiến độ code.
- [ ] **3 dự án seed/dev chờ publish** (`chung-cu-la-bonita`, `du-an-vung-tau`/Silver Sea Tower, `du-an-bay-hien`/Bảy Hiền Tower) — chỉ tồn tại trong `prisma/seed-projects.js`, **không có trong Admin CMS production** (production chỉ có `khu-do-thi-hung-phu`). Nếu/khi publish lên CMS production phải nhập bản dịch EN **trước khi publish** (editor song ngữ đã kiểm chứng runtime — [ADMIN-ITEM-CONTENT-P2 M1](../08-audits-and-reports/current/2026-07-19-admin-item-content-p2-batch-m1.md)).
- [ ] **Xác nhận hết placeholder** — các trang từng là khung placeholder còn `noindex` (`tuyen-dung`, `dao-tao`, `chinh-sach-nhan-su`, `so-do-to-chuc-cong-ty`; `cong-ty-thanh-vien` đã gỡ). Gỡ slug khỏi `placeholderPaths` trong `seo.ts` khi có nội dung thật. Liên đới go-live: **Section 3**.
- [ ] **Ảnh thật cho các dự án / nội dung công ty cấp** — kho ảnh hiện chỉ 1–2 ảnh/dự án nên `ProjectPhotoStrip` chưa trượt; cần ≥ 4 ảnh/dự án (upload qua Admin → tab Hình ảnh) mới tự chạy slide. Ảnh Vista Verde / Feliz en Vista / Hưng Phú Mall thuộc bản quyền CapitaLand / bên thứ ba — **công ty phải cung cấp**, không tự lấy từ Internet. Cần công ty duyệt lại nội dung pháp lý dự án trước go-live (câu 2).

## Hiện trạng kỹ thuật (tham chiếu, cập nhật 2026-07-10)
...
### Việc nên làm trước ở phiên tiếp theo → đã chuyển lên Section 1–3

> Danh sách "việc nên làm trước" cũ (2026-07-10) đã được tách vào cấu trúc mới:
> 1. Áp migration full-text lên production → **Section 3** (Áp migration production có chủ ý).
> 2. Seed trang nội dung → **✅ ĐÃ XONG** (→11, đã chạy production 2026-07-16).
> 3. Cron ngoài cho ED-08 → **Section 3** (ED-08 cron ngoài).
> 4. Nhập bản dịch tiếng Anh CMS → **✅ ĐÃ ĐÓNG** cho phạm vi production/audited (→4); backlog dự án tương lai ở **Section 5**.
> 5. Bấm tay kiểm thử Admin CMS (`BannerFormDialog`/`PageFormDialog`/`BilingualField`) → **Section 3** (UAT thủ công).
> 6. Gửi email form liên hệ → **✅ ĐÃ XONG** (Resend-only, xem Section 0 + →1).
...
## Section 6 — Kho lưu trữ hoàn tất / changelog

> Lịch sử công việc đã hoàn tất, giữ nguyên để truy vết. **Không xoá.** Các mục còn mở từng nằm trong đây (→6, →9, →10, câu 4, câu 5, UAT) đã được chuyển lên Section 1–5; tại vị trí cũ chỉ để lại con trỏ. Thứ tự bên dưới theo tiến trình sprint gốc.
...
- [x] `contact` module: `POST /contact` lưu `contact_submissions` **đã xong** + giới hạn 5 request/IP/giờ **đã xong** (`@Throttle`) + **gửi email thông báo lead đã xong** — `MailService.sendContactNotification` gọi non-blocking trong `contact.service.ts` (thiếu cấu hình mail thì tự no-op, lead vẫn lưu). **Production dùng Resend** (`MAIL_PROVIDER=resend`) và chạy thật; **SMTP fallback đã gỡ khỏi code sau khi Resend ổn định (2026-07-20)** — production chỉ còn Resend, xem mục MANUAL-OPS bên dưới. (YC-09, mục 2.2)
...
- [x] **[MANUAL-OPS]** Cấu hình gửi email thông báo lead trên production — **✅ ĐÃ XONG bằng Resend API** (`MAIL_PROVIDER=resend`). Gmail SMTP đã bỏ vì kết nối SMTP outbound trên Render **timeout** (Render chặn/không ổn định cổng SMTP). Production đặt `resend`. **Cập nhật 2026-07-20 (sau khi Resend ổn định):** SMTP fallback đã **gỡ khỏi code**, biến `SMTP_*` đã **gỡ khỏi Render**, Gmail App Password đã **thu hồi** → email production **chỉ còn Resend**, không còn phương án dự phòng SMTP (xem [SMTP-REMOVAL-ENV-CLEANUP](../08-audits-and-reports/current/2026-07-20-smtp-removal-env-cleanup.md)). Chỉ là nhập cấu hình (không phải việc code); giá trị thật (`RESEND_API_KEY`…) nhập trực tiếp ở Render, không lưu trong Git.
  > ✅ **Ghi nhận hoàn thành (2026-07-20)** — provider: **Resend** · kiểm thử production: **PASS** · email nhận được: **có** (hộp thư Gmail công ty) · phương án dự phòng: **đã gỡ sau khi ổn định** — SMTP fallback loại khỏi code, email production chỉ còn Resend (note EMAIL-RESEND-CLOSEOUT ghi "SMTP giữ lại" theo trạng thái tại thời điểm chốt, đã **superseded** bởi note bên dưới). Chi tiết: [EMAIL-RESEND-CLOSEOUT](../08-audits-and-reports/current/2026-07-20-email-resend-closeout.md) + [SMTP-REMOVAL-ENV-CLEANUP](../08-audits-and-reports/current/2026-07-20-smtp-removal-env-cleanup.md).
...
- [x] Nội dung song ngữ JSONB: **hạ tầng + công cụ nhập liệu xong; bản dịch thật đã nhập qua →4**. `mappers.ts` nhận `locale`, đọc `.en` và **tự fallback về `.vi`** khi thiếu (chuỗi `en` rỗng cũng coi như thiếu). Chuỗi giao diện tĩnh đã dịch trong `dictionaries/en.json`; nhãn nav/footer đánh khóa theo `href` để không phải sửa `data/navigation.ts` / `data/footer.ts`. **Câu 19 đã có trả lời: song ngữ BẮT BUỘC ở go-live** — xem mục Admin CMS bên dưới về ô nhập `en`. Nội dung tiếng Anh do biên tập viên nhập, không tự chế.
  > ✅ **Đóng `[~]`→`[x]` (2026-07-20, đối chiếu →4):** cụm "chưa có bản dịch thật" chỉ đúng ở thời điểm dựng hạ tầng. Bản dịch EN cho **toàn bộ nội dung production/audited** đã hoàn tất qua task →4 (CMS batch 1–4 + loạt C data-model song ngữ + [EN-FULL 7-route](../08-audits-and-reports/current/2026-07-18-en-full-group2-closure.md) + [EN-SITE-WIDE 5-route](../08-audits-and-reports/current/2026-07-18-en-site-wide-follow-up.md) + [EN-PROJECT-ITEMS-P1](../08-audits-and-reports/current/2026-07-18-en-project-items-p1.md)). **Phạm vi mở còn lại là backlog nội dung tương lai** (dự án seed/dev chưa publish — nhập EN trước khi publish), **không chặn go-live** — xem "Backlog nội dung tương lai" ở task →4.
...
> ⏫ **Câu 4 (tuyển dụng / `openPositions`) + Câu 5 (chính sách NS / đào tạo / sơ đồ tổ chức) đã chuyển lên → Section 5 (Backlog nội dung).** Câu 4 giữ `[~]`, Câu 5 giữ `[ ]` FUTURE-CONTENT.
...
## UAT + Go-live — Tuần 10 → đã chuyển lên Section 3

> ⏫ Toàn bộ hạng mục UAT/Go-live (`[ ]`) đã gom vào **Section 3 — Manual ops / production readiness**: checklist nghiệm thu go-live, Lighthouse/k6 trên staging, test khôi phục backup DB (CL-09), HTTPS + domain + DNS (câu 10), UAT thủ công Editor + Admin, xác nhận hết placeholder (câu 1–8). Giữ nguyên nội dung, chỉ đổi vị trí để mọi manual ops nằm chung một chỗ.
...
> ⏫ **(→ 6) Enforce CSP đã chuyển lên → Section 2 (Code kế tiếp — SEC-CSP-ENFORCE-M1).** Giữ `[ ]` mở.
...
> ⏫ **(→ 9) Auth HttpOnly cookie + (→ 10) Staging + WAF đã chuyển lên → Section 4 (Deferred security / infra).** Cả hai giữ `[ ]` mở.
...
- [x] **(→ 12) Bỏ cast `as unknown as Prisma.*Input`** — *Hạng mục: Architecture & engineering depth* ✅ 2026-07-19 (repo + push)
  ...
  - **✅ Đã hoàn thành (2026-07-19):** gỡ **toàn bộ 14 cast** `as unknown as Prisma.*Input` trên 5 service (banners/cooperation/news/pages/projects; trước 14 → sau **0**). Thêm helper **`src/common/prisma-json.ts`** (`json()`) khoanh việc ép cột JSON song ngữ (`en?` optional không khớp `InputJsonValue`) về **một chỗ**; mỗi service dựng payload `{ ...dto, <field JSON>: json(...) } satisfies Prisma.*Input` — `satisfies` bật lại kiểm tra kiểu cho mọi field scalar và **bắt buộc bọc đúng** field JSON. **Giữ nguyên hành vi runtime**: `json()` là identity, spread không rớt field, `normalizeJsonNulls` (projects update) chạy trước rồi mới bọc nên `Prisma.DbNull` đi qua nguyên vẹn; `gallery` (`String[]`, không phải Json) cố ý không bọc. **Không** đổi Prisma schema/DTO/frontend/admin/CI. **Kiểm chứng:** typecheck PASS · `nest build` PASS · lint PASS (0 lỗi; 8 cảnh báo có sẵn ngoài phạm vi) · test 8 suite / 77 pass. Chi tiết: [→12 note](../08-audits-and-reports/current/2026-07-19-prisma-input-casts-removed.md).
...
## Section 7 — Backlog / cải tiến tùy chọn tương lai

> Không chặn go-live. Nhặt lên khi rảnh / khi có nhu cầu. Nguồn chi tiết ở các task Section 6 tương ứng.

- [ ] **Tìm kiếm bỏ dấu (`unaccent`)** — *Nhóm 1/4*: ts config `simple` hiện **không bỏ dấu** (gõ `hung phu` không ra `Hưng Phú`). Bật extension `unaccent` + bọc `unaccent()` trong hai hàm `*_search_document` (cần migration mới + `CREATE EXTENSION unaccent`, xác nhận Render cho phép). Nguồn: YC-10 (Section 6).
- [ ] **Sentry source map upload** — *Nhóm 8*: thêm `SENTRY_AUTH_TOKEN` + build step upload source map để stack trace production đọc được. Đã hoãn tách riêng khỏi →5.
- [ ] **Backup off-site tự động** — *Nhóm 7*: script `pg_dump` + lịch + kho lưu trữ ngoài (bổ sung cho Render managed backup). Chủ dự án đã tách riêng khỏi →2.
- [ ] **G4 — tối ưu hiệu năng còn lại** — *Nhóm 4*: (a) trang chủ mobile LCP 4.3s → tối ưu ảnh banner; (b) TBT 211–317ms trang dự án mobile (chỉ theo dõi); (c) chạy `validator.schema.org` xác nhận chi tiết `Organization`; (d) PSI web re-check sau fix NO_FCP để xác nhận môi trường Google. Nguồn: →13 (Section 6).
- [ ] **`RealEstateListing` / `Product` / `LocalBusiness` / `sameAs`** — *Nhóm 4*: cân nhắc khi công ty mở bán (có giá/offer), có `openingHours`/`geo`, hoặc cung cấp URL mạng xã hội chính thức. Cố ý chưa làm ở →7 (Section 6).
- [ ] **Test Zod schema của 6 FormDialog (Admin)** — *Nhóm 8*: schema module-local, cần quyết định export/testability; validation song ngữ hiện phủ gián tiếp qua `BilingualField`/`bilingual.ts`. Hoãn từ SYS-P1-ADMIN-ROBUSTNESS (Section 6).
